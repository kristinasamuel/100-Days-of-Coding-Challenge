...
let person_Name: string = "Kristina";
...
console.log(
  "titlecase:",
  person_Name.replace(/\bw/g, (c) => c.toUpperCase()) // first big
);
...
let friends: string[] = ["ROOMA", "ROHAN", "SAWARA", "ASAD", "JOHN"];
...
let myFriends: string[] = ["Rooma", "Rohan", "Sawara", "Asad", "shahnzay"];
let messages: string = `Best Of Luck `;
...
let transport: string[] = ["Civic", "Honda", "Suzuki", "Bike"];
...
for (let i = 0; i < shrinkGuestList.length; i++) {
  console.log(
    ` Respected Sir ${shrinkGuestList[i]}  you are still invited on tomorrow dinner.`
  );
}
...
let places: string[] = ["Ameriza", "France", "Bangladesh", "Dubai", "Canada"];
...
let invitedGuestList: string[] = ["Amair", "Ahsan", "Ali", "Junaid"];
...
let book: { name: string; publishIn: number; price: number } = {
  name: "Harry Potter",
  publishIn: 2007,
  price: 2000,
};
console.log(
  `Book Info: Name: ${book.name} , PublishIn: ${book.publishIn} ,price ${book.price}`
);
...
let pizzas: string[] = ["Pepperoni", "Chessy", " Tika Pizza"];
...
let laptop = {
  ...
  describe: function () {
    console.log(
      `the detail of my laptop is: ${laptop.year},${laptop.make},${laptop.model}`
    );
  },
};
laptop.describe();
...
let combinedPrices = [...laptopPrices_1, ...laptopPrices_2].sort(
  (a, b) => a - b
);
...
let averageGrades =
  listOfGrades.reduce((total, listOfGrades) => total + listOfGrades, 0) /
  listOfGrades.length;
...
//  add special number
function Add(valueToAdd: number): (number: any) => number {
